...
module.exports = {
    ...
    async execute(interaction) {
        ...
        // 1. Обработка модалки ГЛОБАЛЬНЫХ настроек сервера (/settings)
        if (customId === "settings_buttons_modal") {
            ...
            try {
                ...
                // Сохраняем новые тексты кнопок в SQLite
        ...
        // 2. Логика управления самой ПРИВАТНОЙ КОМНАТОЙ (лимит / переименование)
        if (customId === "voice_limit_modal" || customId === "voice_rename_modal") {
            ...
            // Получаем настройки сервера, чтобы передать их в рендер панели комнат
            const guildSettings = await getGuildSettings(guildId)

            // Изменение ЛИМИТА участников в комнате
            if (customId === "voice_limit_modal") {
                ...
                try {
                    ...
                    // Передаем guildSettings третьим параметром, чтобы не слетал кастомный текст кнопок
